...
Each is an extension factory (registered at import time with
``kind=ExtensionKind.BACKEND``): handed a tool's callable, name, and
description, it returns a NEW-named branch tool that routes the call through
the RQ queue instead of running it inline. The branch presents the tool's own
signature plus the backend task/schedule options (built with makefun), and the
queued job dispatches back onto the original tool by name via
``tool_execution``.
"""
...
def _wait_for_job_result(job: _PollableJob, timeout: float) -> Any:
    ...
    Raises ``RuntimeError`` when the job vanished or failed (RQ persists a
    failed job's exception only as its traceback text, so that text is carried
    in the raised error — the closest surface to the task's own exception the
    broker offers) and ``TimeoutError`` when ``timeout`` seconds elapse first.
    Blocking Redis polling — callers run it off the event loop.
    """
