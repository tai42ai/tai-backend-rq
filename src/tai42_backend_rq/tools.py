...
``backend_import_schedules`` are the marker tools the host probes for
scheduling availability and the backup round-trip, so their names and
semantics are part of the platform seam.

The RQ scheduler API and ``Job`` accessors are blocking Redis calls, so every
scheduler-touching tool uses the sync Redis client and runs the blocking work
in a thread; the raw-structure tools use the async client directly.
"""
...
def _async_redis(settings: RqSettings) -> AbstractAsyncContextManager[Any]:
    ...
    redis-py shares one command-mixin type stub between its sync and async
    clients, so the async client's awaited return values type-check as the
    sync shapes; the ``Any`` erasure keeps every ``await r...`` call sound.
    """
...
async def _worker_names(r: Any, settings: RqSettings) -> list[str]:
    ...
    RQ's worker registry set stores full worker KEYS (``rq:worker:<name>``);
    the tools report and accept bare names, so the prefix is stripped here.
    """
...
async def _queue_names(r: Any, settings: RqSettings) -> list[str]:
    ...
    Like the worker registry, RQ's queue registry set stores full queue KEYS
    (``rq:queue:<name>``); the tools report bare names and rebuild per-queue
    keys from them, so the prefix is stripped here.
    """
...
async def _read_failure_reason(r: Any, settings: RqSettings, task_id: str) -> str:
    ...
    RQ persists a failed job's exception only as compressed traceback text
    (``exc_string``) in the result stream; when even that is missing, the
    failure carries no retained details.
    """
    ...
    async with _async_redis(settings) as r:
        ...
        while True:
            ...
            if status in (JobStatus.STOPPED, JobStatus.CANCELED):
                # Terminal without a result and without a stored failure —
                # stop waiting and surface it instead of polling until the
                # timeout elapses.
        ...
        # Newest-first: a recurring job re-uses its id, so the stream holds one
        # entry per run and only the LATEST is "the" result.
...
async def backend_list_schedules() -> list[dict[str, Any]]:
    ...
    async with _sync_redis(settings) as r:
        ...
        def _read() -> list[dict[str, Any]]:
            ...
            # with_times pairs each job with its next scheduled run (the zset
            # score), which rq_scheduler returns as a naive-UTC datetime.
    ...
    async with _sync_redis(settings) as r:
        ...
        def _update() -> dict[str, Any]:
            ...
            if new_schedule is not None:
                ...
                # Representability is checked BEFORE the cancel below, so an
                # interval the RQ scheduler cannot hold (see interval_seconds)
                # raises while the prior schedule is still intact.
                new_interval = interval_seconds(norm) if norm["__type__"] == "interval" else None
                # The passthrough meta keeps every custom key but never a stale
                # recurrence key of the replaced kind — the schedule must read
                # back as exactly one kind.
                passthrough_meta = {k: v for k, v in old_meta.items() if k not in ("interval", "cron_string")}

                # Recurrence properties live on the stored job, so the old
                # entry is cancelled and re-created under the same id.
            ...
            if explicit_next_run is not None and not interval_recreated:
                # In-place next-run move; ``old_job`` still carries the right
                # id after a cron re-create. Raises loudly when the schedule
                # vanished rather than silently leaving the time unchanged.
...
async def backend_export_schedules() -> list[dict[str, Any]]:
    ...
    async with _sync_redis(settings) as r:
        ...
        def _read_jobs() -> list[dict[str, Any]]:
            # ``get_jobs`` yields Job objects whose id/args/kwargs/meta each
            # trigger a blocking Redis fetch; materialize every field here so
            # the whole read stays off the event loop rather than only the
    ...
    async with _sync_redis(settings) as r:
        ...
        for index, entry in enumerate(schedules):
            ...
            try:
                ...
                # Re-normalize before any write so a partial/non-canonical
                # schedule is validated and canonicalized.
                ...
                # created vs updated. The create below is id-keyed and
                # overwrites in place, so no pre-cancel is needed and a failure
                # mid-apply leaves the prior schedule intact.
                ...
                # A canonical interval may carry relative=True, which the RQ
                # scheduler cannot represent; the schedule was created above,
                # so surface the dropped flag (only after the apply succeeded —
                # a failed apply must not report the schedule as created).
                ...
                # Collect-and-continue is the section-report contract: one bad
                # record must not abort the batch, and every failure is
                # surfaced in the returned "errors" rows.
