...
``manifest_key`` / ``task_timeout`` / ``tool_name_arg`` mirror the host's
generic backend settings: their defaults must agree with the host's
``BACKEND_``-prefixed group so the tool-dispatch seam meets on the same env
key, timeout, and kwarg name without any configuration.
"""
