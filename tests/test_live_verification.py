...
The event-loop reconciliation this backend owns is that ``run_rq_worker`` runs
the blocking ``worker.work()`` loop on a worker thread, keeping this process's
event loop responsive — the app's worker-bus subscription lives on that loop, so
it keeps applying fleet ops in the worker MAIN process while jobs run. The RQ
process-model observable is that such an apply is inherited by the NEXT job's
forked work-horse child: a child forked mid-op does not see it, and the main
process cannot observe a child mid-flight, so the readback is taken from the next
task rather than the running one.

This test stands in for the bus applier with a main-process registry that a loop
coroutine mutates while the work loop runs on its thread — mutating it at all
proves the loop is not starved — then asserts the next job's forked child reads
the mutated registry. A real ``os.fork()`` work-horse is what makes the
inheritance observable; a main-process-only assertion would pass while a child
served the stale value.

The test is skipped when no Redis is reachable — set ``TAI_TEST_RQ_REDIS``
(default ``redis://localhost:6379/15``) to a running Redis to run it.
"""
