...
``tai42_backend_rq`` is on the allowlist. The rule (see the README): the shipped
package imports
the RQ stack (rq / rq-scheduler / rq-dashboard / redis / gevent), the
shared platform substrate (tai42-contract + tai42-kit, fastmcp, pydantic,
...
The runtime walk imports ``tai42_backend_rq`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_backend_rq`` means the assertion covers the SHIPPED package's
