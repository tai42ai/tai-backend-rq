...
The stub also carries the app facets the source reaches at call time
(``storage.resource_manager`` for template rendering, ``monitoring.active.writer``
for the fork-safety evict, and ``tools.run_tool`` for dispatch); per-test knobs
reset via the autouse ``_reset_stub`` fixture.
"""
